...
//! never stall the PostgreSQL relay hot path.
...
impl ResultCapture {
    fn observe(&mut self, message: &Message) {
        match message.tag {
            b'T' => {
                self.result.sets.push(ResultSet { columns: parse_columns(&message.body), ..ResultSet::default() });
                self.current_set = Some(self.result.sets.len() - 1);
            }
            b'D' => self.capture_row(&message.body),
            b'C' => {
                let command = cstring(&message.body).unwrap_or_default();
                self.row_count = self.row_count.saturating_add(command_row_count(&command));
                self.command_tag = Some(command.clone());
                ...
                self.result.sets[index].command_tag = Some(command);
            ...
            b'E' => {
                ...
                self.error = Some((code, text));
            }
            _ => {}
        }
    }

    fn capture_row(&mut self, body: &[u8]) {
        if self.captured_rows >= MAX_RESULT_ROWS || self.bytes >= MAX_RESULT_BYTES {
            self.truncated = true;
            return;
        }
        let Some(row) = parse_data_row(body) else { return };
...
mod tests {
    ...
    #[test]
    fn backend_errors_are_recorded() {
        let store = TraceStore::memory();
    ...
    fn result_capture_is_bounded() {
        ...
        capture.observe(&row_description(&["value"]));
        for _ in 0..MAX_RESULT_ROWS + 10 {
            capture.observe(&data_row(&[Some("x")]));
        }
