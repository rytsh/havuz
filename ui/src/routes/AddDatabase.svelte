<script lang="ts">
  ...
  import type { Family, PoolMode, SchemaProperty } from "../lib/types";
  ...
  function pick(family: Family) {
    selected = family;
    profileId = family.profiles[0]?.id ?? "";
...
<h1>Add database</h1>
<p class="subtitle">Pick a type, point havuz at it, and set the connection budget.</p>
...
<div class="family-grid">
  {#each families as family (family.id)}
    <button
      class="family-card"
      class:selected={selected?.id === family.id}
      disabled={!family.usable}
      onclick={() => pick(family)}
    >
      <div class="name">
        {family.label}
        {#if !family.usable}<span class="badge">planned</span>{/if}
      </div>
  ...
  <form onsubmit={submit}>
    ...
    {#if selected.profiles.length > 1}
      <div class="field">
        ...
        <div class="help">Same wire protocol, different quirks.</div>
        <select id="profile" bind:value={profileId}>
          {#each selected.profiles as profile (profile.id)}
            <option value={profile.id}>{profile.label} ({profile.maturity})</option>
          {/each}
        </select>
      </div>
    {/if}
